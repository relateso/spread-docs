...
A single automation set consists of the following three elements.  
	...
	2.	**Email**:  [Triggered Campaigns](/spread-email-types#자동화-캠페인-triggered-campaigns)
 or [Sequences](/spread-email-types#시퀀스-sequences)
...
### 2. [Triggered Campaigns](/spread-email-types#자동화-캠페인-triggered-campaigns)

A triggered campaign is a campaign in which pre-written emails are automatically sent when a lead is added to a list.
...
### 3. [Sequences](/spread-email-types#시퀀스-sequences)

A sequence is an individual email flow that continues the conversation with a lead.
...
On the One-off Campaigns, Triggered Campaigns, or Sequences page, refer to the [Send Email Guide](/send-email) and write the email content first.
...
**(3) Send timing (delay)**  : Set when to send the email after an item is added to the list.   
